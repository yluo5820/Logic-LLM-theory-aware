...
class CodeTranslator:
    ...
    def translate_enum_sort_declaration(enum_sort_name, enum_sort_values):
        if all([x.isdigit() for x in enum_sort_values]):
            return [
                CodeTranslator.StdCodeLine(
                    f"{enum_sort_name}_sort = IntSort()", CodeTranslator.LineType.DECL
                )
            ]
    ...
    def translate_int_sort_declaration(int_sort_name, int_sort_values):
        line1 = CodeTranslator.StdCodeLine(
            f"{int_sort_name}_sort = IntSort()", CodeTranslator.LineType.DECL
        )
        line2 = "{} = Ints('{}')".format(
        ...
        line3 = "{} = [{}]".format(
            int_sort_name, ", ".join([str(x) for x in int_sort_values])
        )
    ...
    def handle_count_function(statement):
        ...
        content_end_index = CodeTranslator.extract_paired_token_index(
            statement, content_start_index, "(", ")"
        )
        count_arg_contents = statement[content_start_index + 1 : content_end_index]
        scope_end_index = CodeTranslator.extract_paired_token_index(
            count_arg_contents, 0, "[", "]"
        )
        scope_contents = count_arg_contents[1:scope_end_index]
        vars_and_scopes = CodeTranslator.extract_temperal_variable_name_and_scope(
            scope_contents
        )
        ...
        statement = (
            statement[:index]
            + transformed_count_statement
            + statement[content_end_index + 1 :]
        )
    ...
    def handle_distinct_function(statement):
        ...
        statement = (
            statement[:index]
            + transformed_distinct_statement
            + statement[content_end_index + 1 :]
        )
    ...
    def handle_quantifier_function(statement, scoped_list_to_type):
        ...
        if var_need_declaration:
            for var_name, var_scope in var_need_declaration:
                decl_lines.append(
                    f"{var_name} = Const('{var_name}', {CodeTranslator.type_str_to_type_sort(var_scope)})"
                )
                std_scope.append(var_name)
            std_constraint = "{}([{}], {})".format(
                quant_name, ", ".join(std_scope), quant_expr
            )
        else:
        ...
        if var_need_compresion:
            ...
            std_constraint = "{}([{} {}])".format(
                ...
                " ".join(
                    [
                        f"for {var_name} in {var_scope}"
                        for (var_name, var_scope) in var_need_compresion
                    ]
                ),
            )

        std_constraint = (
            statement[:index] + std_constraint + statement[content_end_index + 1 :]
        )
    ...
    def translate_constraint(constraint, scoped_list_to_type):
        ...
        while re.search(scoped_quantifier_regex, constraint):
            decl_lines, constraint = CodeTranslator.handle_quantifier_function(
                constraint, scoped_list_to_type
            )
            all_decl_lines += decl_lines

        lines = [
            CodeTranslator.StdCodeLine(l, CodeTranslator.LineType.DECL)
            for l in all_decl_lines
        ] + [CodeTranslator.StdCodeLine(constraint, CodeTranslator.LineType.CONS)]
        return lines
            ...
            assert (
                len(
                    [
                        l
                        for l in option_block
                        if l.line_type == CodeTranslator.LineType.CONS
                    ]
                )
                == 1
            )
